...
export function VSCodeLayoutDemo() {
  const { onLayout: onHorizontalLayout } = usePanelSizes("vscode-horizontal-layout");
  const { onLayout: onVerticalLayout } = usePanelSizes("vscode-vertical-layout");
  const sidebarRef = React.useRef<ResizablePanelRef>(null);
  const bottomPanelRef = React.useRef<ResizablePanelRef>(null);
  ...
  const handleBottomToggle = React.useCallback(() => {
  ...
  return (
    <div className="h-[600px] w-full p-4">
      <ResizablePanel.Root
        ...
        autoSaveId="vscode-horizontal-layout"
      ...
      >
        ...
        <ResizablePanel.Panel defaultSize={80}>
          <ResizablePanel.Root 
            ...
            autoSaveId="vscode-vertical-layout"
            className="h-full"
          >
            {/* 编辑器区域 */}
